...
from .environment import WorldSpec, intervention_space_size, isolating_bits
...
def summarize_run(records: list[StepRecord], spec: WorldSpec, budget: int) -> dict:
    ...
    final = records[-1]
    post_shift = [row for row in records if row.observation_index >= spec.shift_observation]
    self_post = [row for row in post_shift if row.domain == "self"]
    need_present_post = [row for row in post_shift if row.need_total > 1e-12]
    sensed_rows = [row for row in post_shift if row.sense_indices]
    sensed_total = 0
    sensed_true_core = 0
    target_sensed = 0
    causal_path_coverage = 0.0
    core_sensing_counts = {index: 0 for index in spec.core_indices}
    for row in sensed_rows:
        sensed = {int(value) for value in row.sense_indices.split(",") if value}
        sensed_total += len(sensed)
        sensed_true_core += len(sensed & set(spec.core_indices))
        target_sensed += int(spec.damaged_index in sensed)
        causal_path_coverage += len(sensed & {spec.mediator_index, spec.damaged_index}) / 2.0
        for index in core_sensing_counts:
            core_sensing_counts[index] += int(index in sensed)
    sensing_rows_count = len(sensed_rows)
    target_sensing_share = target_sensed / sensing_rows_count if sensing_rows_count else 0.0
    other_core = [index for index in spec.core_indices if index != spec.damaged_index]
    other_core_mean_share = (
        mean(core_sensing_counts[index] / sensing_rows_count for index in other_core)
        if sensing_rows_count and other_core else 0.0
    )
    target_sensing_selectivity = target_sensing_share - other_core_mean_share
    ...
    return {
        ...
        "validation_bypassed": int(any(row.validation_bypassed for row in records)),
        ...
        "self_domain_observation_share": len(self_post) / len(post_shift) if post_shift else 0.0,
        "mean_need_target_mass_share": mean(row.need_target_mass_share for row in need_present_post) if need_present_post else 0.0,
        "mean_need_true_core_mass_share": mean(row.need_true_core_mass_share for row in need_present_post) if need_present_post else 0.0,
        "mean_sensed_need_mass_share": mean(row.sensed_need_mass_share for row in need_present_post) if need_present_post else 0.0,
        "need_present_step_share": len(need_present_post) / len(post_shift) if post_shift else 0.0,
        "causal_isolating_observation_share": sum(row.domain == "self" and row.bits == "".join(map(str, isolating_bits(spec.causal_mechanism))) for row in post_shift) / len(post_shift) if post_shift else 0.0,
        "true_core_sensing_share": sensed_true_core / sensed_total if sensed_total else 0.0,
        "causal_target_sensing_share": target_sensing_share,
        "other_core_mean_sensing_share": other_core_mean_share,
        "causal_target_sensing_selectivity": target_sensing_selectivity,
...
def aggregate(rows: list[dict]) -> list[dict]:
    ...
    excluded = {
        "split", "seed", "relevance", "mode", "need_policy", "core_indices", "core_size", "topology",
        "causal_mechanism", "nuisance_mechanism", "true_program", "program_length",
        "grammar_family", "role_program", "label_permuted", "active_roles", "memory_strategy", "nonstationary", "yoke_donor_seed", "yoke_component_shift", "common_decoder_kind",
    }
    output: list[dict] = []
