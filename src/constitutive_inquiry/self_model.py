...
class SelfModelLearner:
    ...
    def sensing_priority(self, needs: list[float], count: int = 8, topology_aware: bool = True) -> tuple[int, ...]:
        """Legacy v0.14 selector retained for reproduction compatibility."""
    ...
    def epistemic_sensing_scores(self, topology_aware: bool = True) -> list[float]:
        """Need-blind value of observing each hidden internal variable.

        The score favors uncertain self-membership, stale measurements, and
        under-sampled variables. It deliberately does not reward being in the
        currently predicted core, because that caused a ceiling where all
        controls repeatedly sensed the damaged target.
        """
        ...
        scores: list[float] = []
        for i in range(N_INTERNAL):
            uncertainty = 1.0 - min(1.0, 2.0 * abs(self.core_probabilities[i] - 0.5))
            staleness = min(1.0, max(0, current - self.last_seen_at[i]) / 10.0)
            under_sampled = 1.0 / math.sqrt(1.0 + max(0, self.probe_counts[i]))
            topology_novelty = 0.0
            if topology_aware and max_cent > 0.0:
                topology_novelty = 1.0 - min(1.0, self.centrality[i] / max_cent)
            scores.append(
                0.42 * uncertainty
                + 0.34 * staleness
                + 0.16 * under_sampled
                + 0.08 * topology_novelty
            )
        return scores

    def epistemic_sensing_priority(self, count: int = 2, topology_aware: bool = True) -> tuple[int, ...]:
        scores = self.epistemic_sensing_scores(topology_aware)
        ranked = sorted(range(N_INTERNAL), key=lambda i: (scores[i], -i), reverse=True)
        return tuple(sorted(ranked[:count]))

    def need_guided_sensing_priority(
        self,
        needs: list[float],
        count: int = 2,
        topology_aware: bool = True,
    ) -> tuple[int, ...]:
        """Dual-channel sensing: maintenance priority plus protected epistemic coverage.

        A purely greedy top-k need selector repeatedly sampled resource depletion and
        could suppress coverage of other damaged components. The first slot is
        need-guided with recency modulation; all remaining slots are selected by the
        same need-blind epistemic rule used by curiosity/no-need controls.
        """

        if len(needs) != N_INTERNAL:
            raise ValueError(f"expected {N_INTERNAL} need components, got {len(needs)}")
        if count <= 0:
            return ()
        epistemic = self.epistemic_sensing_scores(topology_aware)
        maximum = max(needs) if needs else 0.0
        normalized = [value / maximum if maximum > 1e-12 else 0.0 for value in needs]
        current = len(self.states)
        staleness = [min(1.0, max(0, current - self.last_seen_at[i]) / 10.0) for i in range(N_INTERNAL)]
        need_scores = [
            normalized[i] * (0.25 + 0.75 * staleness[i]) + 0.15 * epistemic[i]
            for i in range(N_INTERNAL)
        ]
        need_slot = max(range(N_INTERNAL), key=lambda i: (need_scores[i], -i))
        selected = [need_slot]
        epistemic_ranked = sorted(range(N_INTERNAL), key=lambda i: (epistemic[i], -i), reverse=True)
        for index in epistemic_ranked:
