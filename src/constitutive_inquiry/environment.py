...
class UnlabeledSelfWorld:
    ...
    def __init__(
        ...
        memory_capacity_budget: int | None = None,
    ) -> None:
        ...
        if memory_capacity_budget is not None and memory_capacity_budget < 24:
            raise ValueError("memory_capacity_budget must be at least 24")
        self.memory_capacity_budget = memory_capacity_budget if memory_capacity_budget is not None else max(24, observation_budget)
    ...
    def memory_capacity(self) -> int:
        ...
        reference = self.memory_capacity_budget
        return max(24, min(reference, int(24 + (reference - 24) * value)))
