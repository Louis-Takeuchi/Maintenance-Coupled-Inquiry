...
from .protocol import ConditionSpec, NeedPolicy, SensingPolicy, UNIFIED_SENSING_COUNT
from .repair import select_need_blind_restore_operator
...
class StepRecord:
    ...
    need_total: float
    need_target_mass_share: float
    need_true_core_mass_share: float
    sensed_need_mass_share: float
...
class SideEffectRecord:
    ...
    validation_intervention: tuple[str, ...]
    validation_passed: bool
    validation_effect: float
    validation_bypassed: bool
    repair_attempted: bool
    ...
    repair_correct: bool
    replication_attempted: bool
    replication_intervention: tuple[str, ...]
...
class InquiryAgent:
    def __init__(
        ...
        if condition is None and mode not in MODES:
            raise ValueError(mode)
        if condition is not None and crossworld_memory is not None:
            raise ValueError("unified primary/ablation conditions require memory_policy=off")
        self.mode = mode
        self.condition = condition
        self.record_mode = condition.condition_id if condition is not None else mode
        self.seed = seed
        self.split = split
        self.rng = random.Random(seed + 710_111)
        strategy = condition.self_model_policy.value if condition is not None else ("correlation" if mode.startswith("correlation") else "causal")
        self.self_model = SelfModelLearner(strategy)
        allow_null = condition.allow_null if condition is not None else not mode.startswith("no_null")
        self.model = InquiryModel(allow_null=allow_null)
        self.force_positive = not allow_null
        self.force_validation_pass = condition is None and mode.startswith("no_null")
        self.validation_enabled = condition.bridge_validation if condition is not None else True
        self.observational = condition.passive_only if condition is not None else mode.startswith("observational")
        self.pair_limited = condition.pair_limited if condition is not None else mode.startswith("pair_limited")
        self.common_repair_decoder = condition.common_repair_decoder if condition is not None else False
        self.need_policy = condition.need_policy if condition is not None else None
        self.yoked_trace = yoked_trace or [[0.0] * N_INTERNAL]
        self.crossworld_memory = crossworld_memory
        self.use_memory = condition is None and crossworld_memory is not None and not mode.startswith("reset") and not mode.startswith("oracle")
        self.use_macros = self.use_memory and "macro_disabled" not in mode
        self.use_transition_prior = self.use_macros
        self.records: list[StepRecord] = []
        self.action_history: list[Action] = []
        self.observation_history = []
        self.side_effect_history: list[SideEffectRecord] = []
    ...
    def run(self, world: UnlabeledSelfWorld) -> list[StepRecord]:
        while not world.done:
            ...
            if diagnosis is not None and diagnosis.kind == "bridge" and not self.repaired:
                validation_mechanism = diagnosis.mechanism
                validation_intervention = tuple(diagnosis.intervention)
                if self.validation_enabled:
                    validation_attempted = True
                    validation_passed, validation_effect, _ = world.validate_bridge(diagnosis.mechanism, diagnosis.intervention)
                    if self.force_validation_pass:
                ...
                if validation_passed:
                    ...
                    repair_correct, repair_improvement, restore_operator = self._apply_repair(world, diagnosis, self_prob, needs)
            ...
            need_target_mass_share = (
                needs[world.spec.damaged_index] / need_total if need_total > 1e-12 else 0.0
            )
            need_true_core_mass_share = (
                sum(needs[i] for i in world.spec.core_indices) / need_total if need_total > 1e-12 else 0.0
            )
            sensed_need_mass_share = (
                sum(needs[i] for i in action.sense_indices) / need_total if need_total > 1e-12 else 0.0
            )
            self.records.append(StepRecord(
                ...
                need_policy=(self.need_policy.value if self.need_policy is not None else "legacy"),
                ...
                need_target_mass_share=need_target_mass_share,
                need_true_core_mass_share=need_true_core_mass_share,
                sensed_need_mass_share=sensed_need_mass_share,
            ...
            self.side_effect_history.append(SideEffectRecord(
                validation_attempted=validation_attempted,
                validation_mechanism=validation_mechanism,
                validation_intervention=validation_intervention,
                validation_passed=validation_passed,
                validation_effect=validation_effect,
                validation_bypassed=validation_bypassed,
                repair_attempted=repair_attempted,
                ...
                repair_correct=repair_correct,
                replication_attempted=replication_attempted,
                replication_intervention=tuple(validation_intervention) if replication_attempted else (),
            ))
        return self.records
    ...
    def _needs(self, world: UnlabeledSelfWorld, index: int) -> list[float]:
        if self.condition is not None:
            if self.need_policy in {NeedPolicy.CURIOSITY, NeedPolicy.NONE}:
                return [0.0] * N_INTERNAL
            if self.need_policy == NeedPolicy.YOKED:
                return list(self.yoked_trace[index % len(self.yoked_trace)])
            return self.self_model.need_vector()
    ...
    def _select_action(self, world: UnlabeledSelfWorld, needs: list[float]) -> tuple[Action, float, str]:
        ...
        sensing_count = UNIFIED_SENSING_COUNT if self.condition is not None else 9
        if self.condition is not None:
            if self.condition.sensing_policy == SensingPolicy.NEED_GUIDED:
                self.sensing_panel = self.self_model.need_guided_sensing_priority(
                    needs, sensing_count, topology_aware
                )
            else:
                self.sensing_panel = self.self_model.epistemic_sensing_priority(
                    sensing_count, topology_aware
                )
        else:
            self.sensing_panel = self.self_model.sensing_priority(needs, sensing_count, topology_aware)
        if self.condition is not None:
            if self.need_policy in {NeedPolicy.ACTUAL, NeedPolicy.YOKED}:
                target_share = self._need_target_share(needs)
            elif self.need_policy == NeedPolicy.CURIOSITY:
                target_share = self._curiosity_target_share()
            else:
                target_share = 0.50
        else:
            need = sum(needs)
        ...
        if domain == "neutral":
            bits = self._neutral_bits(index) if self.condition is not None and self.need_policy == NeedPolicy.CURIOSITY else all_bits()[(index * 7 + self.seed) % len(all_bits())]
    ...
    def _need_target_share(self, needs: list[float]) -> float:
        total = sum(needs)
        if total <= 1e-9:
            return 0.50
        self_evidence = self._domain_need_evidence("self", needs)
        neutral_evidence = self._domain_need_evidence("neutral", needs)
        contrast = self_evidence - neutral_evidence
        urgency = math.tanh(3.0 * total)
        return max(0.30, min(0.78, 0.50 + 0.16 * urgency + 2.4 * contrast))

    def _domain_need_evidence(self, domain: str, needs: list[float]) -> float:
        total = sum(needs)
        if total <= 1e-9:
            return 0.0
        weights = [value / total for value in needs]
        rows: list[float] = []
        for observation in self.model.observations[-48:]:
            if observation.domain != domain:
                continue
            numerator = 0.0
            denominator = 0.0
            for i in observation.observed_indices:
                before = observation.internal_before[i]
                after = observation.internal_after[i]
                if before is None or after is None or weights[i] <= 0.0:
                    continue
                numerator += weights[i] * max(0.0, float(before) - float(after))
                denominator += weights[i]
            if denominator > 0.0:
                rows.append(numerator / denominator)
        return sum(rows) / len(rows) if rows else 0.0

    def _curiosity_target_share(self) -> float:
        self_score = self._domain_uncertainty("self")
        neutral_score = self._domain_uncertainty("neutral")
        total = self_score + neutral_score
        return 0.50 if total <= 1e-9 else max(0.30, min(0.70, self_score / total))

    def _domain_uncertainty(self, domain: str) -> float:
        rows = [observation for observation in self.model.observations if observation.domain == domain]
        if not rows:
            return 1.0
        rate = sum(observation.residual for observation in rows) / len(rows)
        entropy = 0.0
        for probability in (rate, 1.0 - rate):
            if probability > 1e-12:
                entropy -= probability * math.log(probability)
        coverage = len({observation.bits for observation in rows}) / len(all_bits())
        return 0.55 * entropy + 0.45 * (1.0 - coverage) + 1.0 / math.sqrt(1.0 + len(rows))

    def _neutral_bits(self, index: int) -> tuple[int, int, int, int]:
        counts = {bits: 0 for bits in all_bits()}
        for observation in self.model.observations:
            if observation.domain == "neutral":
                counts[observation.bits] += 1
        minimum = min(counts.values())
        candidates = [bits for bits, count in counts.items() if count == minimum]
        return candidates[(index + self.seed) % len(candidates)]
    ...
    def _self_scientific_action(self, world: UnlabeledSelfWorld) -> tuple[Action, str]:
        ...
        max_depth = 2 if self.pair_limited else MAX_PROGRAM_LENGTH
    ...
    def _apply_repair(self, world: UnlabeledSelfWorld, diagnosis: Diagnosis, self_prob: list[float], needs: list[float]) -> tuple[bool, float, str]:
        if self.common_repair_decoder:
            best_operator = select_need_blind_restore_operator(
                world, diagnosis.target_index, self_prob
            )
        else:
